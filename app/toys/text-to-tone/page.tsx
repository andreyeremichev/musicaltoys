...
if (isPlaying || overLimit) return;
  ...
  if (!localEvts.length) return;

  // ===== DEBUG: LIVE AUDIO (TextToTone) =====
try {
  const labels = (localEvts as any[]).map((ev) => labelForEventAminor(ev as any));

  logLiveAudioDebug({
    page: "text-to-tone",
    tag: `mode=${mode}`,      // "letters" / "ipa"
    noteNames: labels,        // will be "a b c" etc
  });
} catch (err) {
  console.warn("[TextToTone LIVE DEBUG] failed to log", err);
}
...
// 2b) DEBUG: EXPORT AUDIO (notes actually going into export)
try {
  // Inspect raw structure of one event
  if (evtsExport[0]) {
    console.log("[T2T EXPORT RAW SAMPLE]", evtsExport[0]);
    console.log("[T2T EXPORT KEYS]", Object.keys(evtsExport[0] as any));
  }

  const midiToNoteDebug = (midi: number) => {
    const NAMES = [
  ...
  const exportNoteNames = (evtsExport as any[]).map((ev, idx) => {
    const e: any = ev;
    if (e.noteName) return String(e.noteName);
    if (e.note) return String(e.note);
    if (e.pitch) return String(e.pitch);
    if (typeof e.midi === "number") return midiToNoteDebug(e.midi);
    if (typeof e.pc === "string" && typeof e.oct === "number") return `${e.pc}${e.oct}`;
    return `ev${idx}`;
  });
  ...
  logExportAudioDebug({
    page: "text-to-tone",
    tag: `mode=${mode}`,
    degreesInput: exportInput,
    noteEventsToExport: exportNoteNames,
  });
} catch (err) {
  console.warn("[TextToTone EXPORT DEBUG] failed to log", err);
}

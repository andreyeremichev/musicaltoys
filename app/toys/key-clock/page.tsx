...
/* =========================
   ...
   (NO UI for these)
========================= */
// 1) Second caption line (degrees/chords labels) — OFF by default.
...
function midiToNoteName(midi: number): NoteName {
  ...
  const noteName = `${pc}${oct}` as NoteName;

  if (DEBUG_LIVE_AUDIO) {
    console.log("[KeyClock midiToNoteName]", midi, "→", noteName);
  }

  return noteName;
}
async function loadBuffer(noteName: string): Promise<AudioBuffer> {
  if (_buffers.has(noteName)) {
    if (DEBUG_LIVE_AUDIO) {
      console.log("[KeyClock loadBuffer cache hit]", noteName);
    }
    return _buffers.get(noteName)!;
  }

  const safe = noteName.replace("#", "%23");

  if (DEBUG_LIVE_AUDIO) {
    console.log("[KeyClock loadBuffer fetch]", `/audio/notes/${safe}.wav`);
  }
...
export default function KeyClockPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const q = sp.get("q");
      if (q) setRaw(sanitizeKeyClockInput(q));
  ...
  // Keep existing sanitization behavior used by input (as in current working file)
  ...
  const [passMode, setPassMode] = useState<PassMode>("2M2m"); // 5) Major/Minor picker (default = current)
  ...
  // 2) Ring toggle "R" — OFF by default
  const [showRing, setShowRing] = useState(false);
  // 3) Helper toggle "H" — OFF by default (controls node labels near nodes)
  ...
  /* =========================
     Caption building (unchanged helpers)
  ========================= */
  ...
  /* Helpers */
  function appendTrail(spoke: number, key: KeyName, colorMaj: string = T.gold, colorMin: string = T.minor) {
  ...
  /* =========================
     Live caption (two lines, bottom line gated by in-file constant)
  ========================= */
  ...
  /* =========================
     ...
     - keep mp4 logic unchanged
     - bottom caption gated by in-file constant
  ========================= */
  const onDownloadVideo = useCallback(async () => {
    ...
    try {
      ...
      // 4) Schedule (based on selected pass mode)
      const NOTE_MS_E = 250;
      ...
      for (let i = 0; i < STEPS_E; i++) {
        ...
        if (tok.kind === "resolve") {
          scheduleResolveCadenceE(at, keyNowE);
          continue;
        }
        if (tok.kind === "rest") {
          ...
          if (isLetter) {
            if (tok.kind === "deg") {
              if (tok.up && tok.src === "8") {
                ...
                const spoke = degToIndexForKey("1", keyNowE);
                ...
                const spoke = degToIndexForKey(tok.d, keyNowE);
                if (spoke >= 0) exportPoints.push({ k: kTag, step: i, spoke });
              }
            } else {
            ...
            if (tok.kind === "deg") {
              if ("1234567".includes(tok.src)) {
                ...
                const spoke = degToIndexForKey("2", keyNowE);
                if (spoke >= 0) exportPoints.push({ k: kTag, step: i, spoke });
              }
      ...
      function overlaySvgForStep(stepIdx: number): string {
        ...
        return `
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="${liveW}" height="${liveH}" shape-rendering="geometricPrecision">
            ...
            ${
              pathMaj
                ? `<path d="${pathMaj}" fill="none" stroke="${T.gold}" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" ${
                    trailMode.startsWith("glow") ? `filter="url(#vt-glow)"` : ""
                  } />`
                : ""
            }
            ${
              pathMin
                ? `<path d="${pathMin}" fill="none" stroke="${T.minor}" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" ${
                    trailMode.startsWith("glow") ? `filter="url(#vt-glow)"` : ""
                  } />`
                : ""
            }
      ...
      (function loop() {
        ...
        c.lineWidth = 2 * SCALE;
        c.strokeRect((SIDE_PAD / 2) * SCALE, SAFE_TOP * SCALE, (FRAME_W - SIDE_PAD) * SCALE, (drawH + titleBlockH + TOP_GAP) * SCALE);
        c.restore();

        c.drawImage(bgImg, 0, 0, liveW, liveH, drawX * SCALE, (goldTop + 12) * SCALE, drawW * SCALE, drawH * SCALE);
  ...
  /* =========================
     ...
     - now uses selected SEGMENTS
  ========================= */
  const startCore = useCallback(async () => {
    const NOTE_STEP = 0.25;
    ...
    for (let i = 0; i < STEPS; i++) {
      ...
      debugNoteNames.push(`${curKey}:${tok && typeof tok === "object" ? JSON.stringify(tok) : String(tok ?? "")}`);
      ...
      if (step > lastDrawnStep) {
        for (let s = lastDrawnStep + 1; s <= step; s++) {
          ...
          if (spoke >= 0) {
            ...
            const isPlayable = tok.kind === "deg" || tok.kind === "chroma";
  ...
  /* =========================
     Node label overrides (ONLY 4 labels)
========================= */
  function nodeLabelOverride(lab: DegLabel): { text: string; isSpecial: boolean } {
    // 4) Change ONLY these labels; all others remain as-is.
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              description: "Turn dates and times into musical cadences. Three zero modes: Chromatic, Ticks, Rest.",
              ...
              mainEntity: [
                {
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: "KeyClock maps dates and times to musical cadences in B♭ Major and C minor across passes.",
                  },
                },
                {
                  "@type": "Question",
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: "Choose Chromatic (♭2/♯4), Ticks (short ♭2/♯4), or Rest (silent). Chromatic/Ticks are playable and highlighted; Rest is silent and unhighlighted.",
                  },
                },
              ],
            }),
  ...
  style={{
    ...
    // 🔧 spacing tweaks
    marginTop: -2,     // input → caption
    marginBottom: -20,  // caption → circle
  }}
>
  <canvas
...
>
  ...
  >
          ...
          <div className="vt-actions minw0" aria-label="Actions" style={{ display: "flex", justifyContent: "center", gap: 10, flexWrap: "wrap" }}>
            {isExporting && (
              <div style={{ color: T.muted, fontSize: 12, textAlign: "center", width: "100%", marginTop: 6 }}>⏺️ Recording…</div>
            )}
            <button
              onClick={() => start()}
              disabled={!raw.trim()}
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: `1px solid ${T.border}`,
                  background: showHelper ? (bg === "dark" ? "#0F1821" : "#F3F5F8") : "transparent",
                  color: T.muted,
                  fontWeight: 800,
                  cursor: "pointer",
                  lineHeight: "30px",

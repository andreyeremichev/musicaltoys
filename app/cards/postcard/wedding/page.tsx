...
/* =========================================================
   Wedding Musical Postcard — LIVE TRAILS + WISH MAGIC + AUDIO + INPUT CAPTION
...
// Wedding atmosphere choices (intro layer, user can toggle off)
type AtmospherePreset = "bright" | "long";

// Wedding presets (01–08)
const LAYOUTS: Record<
  LayoutId,
  {
    ...
    defaultAtmosphereOn: boolean;
    defaultAtmospherePreset: AtmospherePreset; // "bright" | "long"
    allowedAtmospheres: AtmospherePreset[];

    demo: { style: DemoStyle; bursts?: number };
  }
> = {
  // 01 — White cake (soft + elegant)
  "01": {
    label: "Wedding Cake",
    src: "/cards/postcards/wedding/01.jpg",
    wishFont: "vintage",
    disableBackdrop: true,
    defaults: { textInk: "warmWhite", trailA: "antiqueGold", trailB: "rose" },
    defaultMotion: "glow",
  ...
  // 02 — White rose (pure + calm)
  "02": {
    label: "White Rose",
    src: "/cards/postcards/wedding/02.jpg",
    wishFont: "vintage",
    disableBackdrop: true,
    defaults: { textInk: "cocoa", trailA: "antiqueGold", trailB: "frost" },
    defaultMotion: "glow",
    defaultMood: "brighter",
    defaultZeroPolicy: "ticks",
    defaultAtmosphereOn: true,
  ...
  // 03 — Champagne close-up (sparkle, but still classy)
  "03": {
    label: "Champagne",
    src: "/cards/postcards/wedding/03.jpg",
    wishFont: "vintage",
    disableBackdrop: true,
    defaults: { textInk: "warmWhite", trailA: "gold", trailB: "sky" },
    defaultMotion: "pulse",
    defaultMood: "brighter",
    defaultZeroPolicy: "ticks",
    defaultAtmosphereOn: true,
  ...
  // 04 — Cupcake with rings (sweet + celebratory)
  "04": {
    label: "Rings + Candle",
    src: "/cards/postcards/wedding/04.jpg",
    wishFont: "hand",
    disableBackdrop: true,
    defaults: { textInk: "ink", trailA: "gold", trailB: "rose" },
    defaultMotion: "pulse",
    defaultMood: "brighter",
    defaultZeroPolicy: "ticks",
    defaultAtmosphereOn: true,
  ...
  // 05 — Rings macro (minimal, timeless)
  "05": {
    label: "The Ring",
    src: "/cards/postcards/wedding/05.jpg",
    wishFont: "vintage",
    disableBackdrop: true,
    defaults: { textInk: "cocoa", trailA: "antiqueGold", trailB: "gold" },
    defaultMotion: "glow",
  ...
  // 06 — Toast + confetti (the “party” one)
  "06": {
    label: "Toast",
    src: "/cards/postcards/wedding/06.jpg",
    wishFont: "vintage",
    disableBackdrop: true,
    defaults: { textInk: "warmWhite", trailA: "gold", trailB: "rose" },
    defaultMotion: "pulse",
    defaultMood: "brighter",
    defaultZeroPolicy: "ticks",
    defaultAtmosphereOn: true,
  ...
  // 07 — Confetti rose (soft romance + shimmer)
  "07": {
    label: "Rose Confetti",
    src: "/cards/postcards/wedding/07.jpg",
    wishFont: "hand",
    disableBackdrop: true,
    defaults: { textInk: "cocoa", trailA: "rose", trailB: "antiqueGold" },
    defaultMotion: "glow",
    defaultMood: "balanced",
  ...
  // 08 — Lace detail (quiet, intimate)
  "08": {
    label: "Lace",
    src: "/cards/postcards/wedding/08.jpg",
    wishFont: "vintage",
    disableBackdrop: true,
    defaults: { textInk: "cocoa", trailA: "antiqueGold", trailB: "frost" },
    defaultMotion: "glow",
...
const CUSTOM_WISH_LABEL = "Your words…";
...
function totalForDigitRun(n: number): number {
  // Date-we-met: digits should breathe more than letters
  if (n <= 1) return 1.35 * STEP_MS;
  if (n === 2) return 1.60 * STEP_MS;
  if (n === 3) return 1.95 * STEP_MS;
  if (n === 4) return 2.25 * STEP_MS;

  const scaled = (1.7 + 0.22 * (n - 2)) * STEP_MS;
  return Math.min(2.60 * STEP_MS, scaled);
}
...
function pickRecorderMime(): string {
  const candidates = [
    // ✅ WebM first — reliable audio
    "video/webm;codecs=vp9,opus",
    "video/webm;codecs=vp8,opus",
    "video/webm",

    // ❌ MP4 LAST — often video-only
...
export default function XmasPostcardPage() {
  ...
  const [typed, setTyped] = useState("2026 we start");
...
const [demoRevealOn, setDemoRevealOn] = useState(false);
  ...
  // Atmosphere (Wedding intro sound)
const [atmosphereOn, setAtmosphereOn] = useState(false);
const [atmospherePreset, setAtmospherePreset] = useState<AtmospherePreset>("bright");
...
setAtmosphereOn((layout as any).defaultAtmosphereOn ?? false);
setAtmospherePreset((layout as any).defaultAtmospherePreset ?? "bright");
...
const fireworksRafRef = useRef<number>(0);
  ...
  /* -------------------- Audio scheduling helpers (KeyClock-correct) -------------------- */
  ...
  loadBuffer(noteName)
    .then((buf) => {
      ...
      if (env === "tick") {
        ...
        g.gain.linearRampToValueAtTime(0.45 * gainMax, when + 0.006);
        ...
        g.gain.setTargetAtTime(0, when + 0.2, 0.05);
        src.connect(g).connect(ac.destination);
        try {
          src.start(when);
          src.stop(when + dur);
...
function scheduleNoteLive(
  ...
  playBufferWithEnv(ac, at, name, "normal", gainMax, dur);
}

function scheduleIntroChord(ac: AudioContext, at: number, key: KeyName) {
  (["1", "3", "5"] as Diatonic[]).forEach((d, i) => {
    scheduleNoteLive(ac, at + i * 0.06, d, key, false, 0.25, 0.9);
  });
}
function scheduleAtmosphereIntro(
  ac: AudioContext,
  at: number,
  key: KeyName,
  preset: AtmospherePreset
) {
  // Local helper with controllable duration so "Long" is actually long.
  const playAt = (when: number, midi: number, gainMax: number, durSec: number) => {
    const name = midiToNoteName(midi);
    loadBuffer(name)
      .then((buf) => {
        const src = ac.createBufferSource();
        ...
        // Normal envelope (KeyClock-like) with custom stop time
        g.gain.setValueAtTime(0, when);
        g.gain.linearRampToValueAtTime(1 * gainMax, when + 0.01);
        g.gain.setTargetAtTime(0, when + Math.min(0.35, durSec * 0.6), 0.10);

        src.connect(g).connect(ac.destination);
        try {
          src.start(when);
          src.stop(when + durSec);
        } catch {}
      })
      .catch(() => {});
  };

  // High notes feel “bright”
  const h1 = degreeToMidi("1", key, true);
  const h3 = degreeToMidi("3", key, true);
  const h5 = degreeToMidi("5", key, true);

  // Low notes feel “warm/long”
  const l1 = degreeToMidi("1", key, false) - 12;
  const l3 = degreeToMidi("3", key, false) - 12;
  const l5 = degreeToMidi("5", key, false) - 12;

  if (preset === "long") {
    // LONG: low, warm, sustained (very obvious difference)
    playAt(at + 0.00, l1, 0.45, 0.85);
    playAt(at + 0.00, l3, 0.38, 0.85);
    playAt(at + 0.00, l5, 0.35, 0.85);
    return;
  }

  // BRIGHT (default): high, short, stepping (very obvious difference)
  playAt(at + 0.00, h1, 0.85, 0.22);
  playAt(at + 0.08, h3, 0.78, 0.22);
  playAt(at + 0.16, h5, 0.70, 0.22);
}

function scheduleResolveCadence(ac: AudioContext, at: number, key: KeyName) {
  scheduleNoteLive(ac, at, "5", key, false, 0.25, 0.9);
  scheduleNoteLive(ac, at + 0.12, "1", key, false, 0.25, 0.9);
}

function scheduleTriad(ac: AudioContext, at: number, root: Diatonic, key: KeyName, dur = 0.25) {
  const wrap = (n: number) => (["1", "2", "3", "4", "5", "6", "7"][(n - 1 + 7) % 7] as Diatonic);
  const r = Number(root);
  const ds: Diatonic[] = [wrap(r), wrap(r + 2), wrap(r + 4)];

  // KeyClock behavior: chord tones start together
  ds.forEach((d) => scheduleNoteLive(ac, at, d, key, false, dur, 0.9));
}

function scheduleFirstInvTonic(ac: AudioContext, at: number, key: KeyName, dur = 0.25) {
  scheduleNoteLive(ac, at, "3", key, false, dur, 0.9);
  scheduleNoteLive(ac, at, "5", key, false, dur, 0.9);
  scheduleNoteLive(ac, at, "1", key, true, dur, 0.9);
}

function scheduleSecondInvSupertonic(ac: AudioContext, at: number, key: KeyName, dur = 0.25) {
  scheduleNoteLive(ac, at, "6", key, false, dur, 0.9);
  scheduleNoteLive(ac, at, "2", key, true, dur, 0.9);
  scheduleNoteLive(ac, at, "4", key, true, dur, 0.9);
}

function scheduleChromaLive(
  ac: AudioContext,
  at: number,
  tok: Extract<Token, { kind: "chroma" }>,
  key: KeyName
) {
...
function scheduleLetterDeg(ac: AudioContext, at: number, tok: Extract<Token, { kind: "deg" }>, key: KeyName) {
  // KeyClock behavior for letter-derived 8/9:
  if (tok.up && tok.src === "8") {
    scheduleNoteLive(ac, at, "1", key, false, 0.25, 0.9);
    scheduleNoteLive(ac, at, "1", key, true, 0.25, 0.9);
    return;
  }
  if (tok.up && tok.src === "9") {
    scheduleNoteLive(ac, at, "1", key, false, 0.25, 0.9);
    scheduleNoteLive(ac, at, "2", key, true, 0.25, 0.9);
    return;
  }
  scheduleNoteLive(ac, at, tok.d, key, tok.up, 0.25, 0.9);
}
const playCore = useCallback(async () => {
  ...
  const cum: number[] = new Array(segLen + 1);
  cum[0] = 0;
  ...
  // ---- Live start times (separate intro from main phrase)
const tIntro = ac.currentTime + 0.12;

const introPadSec =
  atmosphereOn
    ? (atmospherePreset === "long" ? 1.15 : 0.80)
    : 0;

const t0 = atmosphereOn ? (tIntro + introPadSec) : (ac.currentTime + 0.12);
if (atmosphereOn) {
  // intro uses the first segment key
  scheduleAtmosphereIntro(ac, tIntro, segments[0] ?? "BbMajor", atmospherePreset);
}
  // ✅ Schedule audio (THIS WAS MISSING)
for (let segIdx = 0; segIdx < segCount; segIdx++) {
  const keyNow: KeyName = segments[segIdx];
  const segBaseSec = t0 + (segIdx * segTotalMs) / 1000;

  // Wedding-style atmosphere: play once at the very start (only first segment)
if (atmosphereOn && segIdx === 0) {
  // Use the first segment’s key for the intro
  scheduleAtmosphereIntro(ac, tIntro, keyNow, atmospherePreset);
}
  ...
  for (let i = 0; i < segLen; i++) {
    ...
    if (tok.kind === "deg") {
      if (isLetter) {
        scheduleLetterDeg(ac, at, tok, keyNow);
      } else {
        if ("1234567".includes(tok.src)) scheduleTriad(ac, at, tok.d, keyNow, 0.16);
        else if (tok.src === "8") scheduleFirstInvTonic(ac, at, keyNow, 0.16);
        else if (tok.src === "9") scheduleSecondInvSupertonic(ac, at, keyNow, 0.16);
      }
  ...
  // Keep your existing "Schedule audio for segIdx/idxInSeg" code exactly as you already have it.
...
atmospherePreset,
  ...
  // ✅ IMPORTANT: unlock audio while we still have the user gesture
  await unlockCtx();
    primeAudioOutput(); // ✅ helps Safari / strict gesture policies


  // then do the wish flash and start the engine after
  runWishFlash(wish, () => void playCore());
}, [cancelSparkles, cancelWishAnim, runWishFlash, wish, playCore]);
  ...
  setWishIdx((v) => {
    ...
    runWishFlash(
      nextChoice === CUSTOM_WISH_LABEL ? (customWish.trim() || CUSTOM_WISH_LABEL) : nextChoice
    );

    return next;
  });
}, [cancelSparkles, cancelWishAnim, runWishFlash, customWish]);
...
try { void getCtx(); } catch {}
    ...
    // --- Export start times (separate intro from main phrase) ---
  ...
  // buffer MUST already be loaded by the preload step below
  ...
  if (mode === "tick") {
    g.gain.setValueAtTime(0, at);
    g.gain.linearRampToValueAtTime(1.25 * gainMax, at + 0.006);
  ...
  g.connect(ac.destination);
const stopAfter = dur ?? (mode === "tick" ? 0.20 : 0.30);
...
// -------------------- PRELOAD BUFFERS (CRITICAL FOR EXPORT AUDIO) --------------------
const preloadNames = new Set<string>();

// Tick source note (your tick uses C4 in the toy)
preloadNames.add(midiToNoteName(60));

// Atmosphere intro notes (both presets)
{
  const key0: KeyName = segments[0] ?? "BbMajor";

  // High (bright)
  preloadNames.add(midiToNoteName(degreeToMidi("1", key0, true)));
  preloadNames.add(midiToNoteName(degreeToMidi("3", key0, true)));
  preloadNames.add(midiToNoteName(degreeToMidi("5", key0, true)));

  // Low (long)
  preloadNames.add(midiToNoteName(degreeToMidi("1", key0, false) - 12));
  preloadNames.add(midiToNoteName(degreeToMidi("3", key0, false) - 12));
  preloadNames.add(midiToNoteName(degreeToMidi("5", key0, false) - 12));
}

// Also preload everything needed by the export run (safe, fast because cached)
for (let segIdx = 0; segIdx < segments.length; segIdx++) {
  ...
  for (let i = 0; i < schedule.length; i++) {
    ...
    // resolve chord uses degrees 5 and 1
    if (tok.kind === "resolve") {
    ...
    if (tok.kind === "deg") {
      ...
      if (isLetter) {
        // letter: single note (plus the special 8/9 behavior)
        if (tok.up && tok.src === "8") {
          preloadNames.add(midiToNoteName(degreeToMidi("1", keyNow, false)));
        ...
        // digits: triad/inversions
        if ("1234567".includes(tok.src)) {
          // triad degrees: root, root+2, root+4
          const wrap = (n: number) => (["1","2","3","4","5","6","7"][(n - 1 + 7) % 7] as Diatonic);
...
// Decode everything now (once)
await Promise.all(Array.from(preloadNames).map((n) => loadBuffer(n)));
// -------------------- Atmosphere intro (export) --------------------
const scheduleAtmosphereIntroE = (at: number, key: KeyName) => {
  if (!atmosphereOn) return;

  // High (Bright)
  const h1 = midiToNoteName(degreeToMidi("1", key, true));
  const h3 = midiToNoteName(degreeToMidi("3", key, true));
  const h5 = midiToNoteName(degreeToMidi("5", key, true));

  // Low (Long)
  const l1 = midiToNoteName(degreeToMidi("1", key, false) - 12);
  const l3 = midiToNoteName(degreeToMidi("3", key, false) - 12);
  const l5 = midiToNoteName(degreeToMidi("5", key, false) - 12);

  if (atmospherePreset === "long") {
    // Long: sustained warm wash (clearly audible)
    playExportOneShot(at + 0.00, l1, "normal", 0.45, 0.95);
    playExportOneShot(at + 0.00, l3, "normal", 0.38, 0.95);
    playExportOneShot(at + 0.00, l5, "normal", 0.35, 0.95);
    return;
  }

  // Bright: short stepping sparkle (clearly separated)
  playExportOneShot(at + 0.00, h1, "normal", 0.85, 0.30);
  playExportOneShot(at + 0.08, h3, "normal", 0.78, 0.30);
  playExportOneShot(at + 0.16, h5, "normal", 0.70, 0.30);
};

// -------------------- Export scheduling helpers (use playExportOneShot ONLY) --------------------
...
const scheduleTriadE = (at: number, root: Diatonic, key: KeyName) => {
  const wrap = (n: number) =>
    (["1", "2", "3", "4", "5", "6", "7"][(n - 1 + 7) % 7] as Diatonic);
  const r = Number(root);
...
const scheduleChromaE = (at: number, tok: Extract<Token, { kind: "chroma" }>, key: KeyName) => {
  ...
  if (useTickEnv) {
  // louder & slightly longer so ticks survive export
  playExportOneShot(at, name, "tick", 1.0, 0.20);
} else {
  playExportOneShot(at, name, "normal", 0.85);
}
};
...
// If atmosphere is ON, we give it a short intro window before the main phrase.
const tIntro = ac.currentTime + 1.6;

// how long the intro should "own" the timeline before the date starts
const introPadSec =
  atmosphereOn
    ? (atmospherePreset === "long" ? 1.30 : 0.90) // long needs more space than bright
    : 0;

const t0 = atmosphereOn ? (tIntro + introPadSec) : (ac.currentTime + 1.0);



scheduleAtmosphereIntroE(tIntro, segments[0] ?? "BbMajor");

// -------------------- Schedule export audio (main phrase) --------------------
const hasIntro = tokens.some((t) => t.kind === "intro");
...
for (let segIdx = 0; segIdx < segCount; segIdx++) {
  ...
  for (let i = 0; i < segLen; i++) {
    ...
    if (tok.kind === "deg") {
      if (isLetter) {
        scheduleLetterDegE(at, tok, keyNow);
      } else {
...
// --- Visual sync: align export visuals to audio start (t0) ---
const acNowAtStart = ac.currentTime;
const msUntilMain = Math.max(0, (t0 - acNowAtStart) * 1000);
   ...
   let lastStepDrawn = -1;
    ...
    const drawFrame = () => {
      ...
      // compute step by MAIN-PHRASE time (aligned to audio start t0)
const elapsedMainMs = elapsedWall - msUntilMain;
const mainStarted = elapsedMainMs >= 0;
// ✅ One-time reset when main audio begins (fixes messy first spiral pass)
if (mainStarted && !didStartMain) {
  didStartMain = true;

  // reset step advancing so first visible step starts clean
  lastStepDrawn = -1;

  // reset spiral state so pass #1 starts from a clean center
  if (isSingleMode) {
    startSpiralPass();
  }

  // (optional but safe) also clear balanced nodes so nothing leaks
  majNodes.length = 0;
  minNodes.length = 0;

  // (optional) clear fireworks particles if you’re in pulse mode
  fw.length = 0;
}

// Done when waiting time + main phrase duration is complete
const done = elapsedWall >= (msUntilMain + totalMs);

// Main timeline is 0..totalMs (starts exactly when audio main phrase starts)
const tMain = Math.max(0, elapsedMainMs);

const segIdx = Math.floor(tMain / segTotalMs);
const inSeg = tMain - segIdx * segTotalMs;

let idxInSeg = 0;
while (idxInSeg + 1 < cum.length && cum[idxInSeg + 1] <= inSeg) idxInSeg++;

const globalStep = mainStarted
  ? clamp(segIdx, 0, segCount - 1) * segLen + idxInSeg
  : -1;
...
if (mainStarted) {
  ...
  // ✅ Export burst fix: only spawn for the newest step (prevents "bubble stream")
  if (lastStepDrawn === globalStep) {
    const p = nodePosition(spoke, 36);
...
if (showPulseOnly && mainStarted && fw.length === 0 && globalStep >= 0) {
  // force one burst at center so we know draw works
  spawnFw(50, 50, true);
}
  ...
  dangerouslySetInnerHTML={{
    __html: JSON.stringify({
      ...
      "@type": "WebApplication",
      name: "Wedding Musical Postcard",
      applicationCategory: "MusicApplication",
      operatingSystem: "Web",
      url: "https://musicaltoys.app/cards/postcard/Wedding",
      description:
        "Create a ‘Wedding’ musical postcard from a meaningful date or message. Choose a background, play it, and share a link that recreates the same postcard.",
...
/>
        <header style={{ textAlign: "center", paddingTop: 2 }}>
          <div style={{ fontSize: 17, fontWeight: 950, letterSpacing: "0.01em" }}>Wedding Postcard</div>
...
<div style={{ display: "grid", gap: 8 }}>
  <div style={{ fontSize: 12, fontWeight: 900, opacity: 0.8 }}>Atmosphere</div>

  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
    <button
      type="button"
      onClick={() => setAtmosphereOn(true)}
      style={pill(atmosphereOn)}
      title="Extra Wedding intro sound"
    >
      On
    </button>
    <button
      type="button"
      onClick={() => setAtmosphereOn(false)}
      style={pill(!atmosphereOn)}
      title="No extra intro"
    >
      Off
    </button>
  </div>
  ...
  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
    <button
      type="button"
      onClick={() => setAtmospherePreset("bright")}
      style={pill(atmospherePreset === "bright")}
    >
      Bright
    </button>
    <button
      type="button"
      onClick={() => setAtmospherePreset("long")}
      style={pill(atmospherePreset === "long")}
    >
      Long
    </button>
  </div>
  ...
  <div style={{ fontSize: 11, opacity: 0.55 }}>
    A tiny intro sound before the date plays.
  </div>
</div>
                <div style={{ display: "grid", gap: 6 }}>
